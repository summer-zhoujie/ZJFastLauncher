...
public class PathInterpolatorCompat implements Interpolator {
    ...
    private void initPath(Path path) {
        ...
        if (min > 0) {
            this.f25mX = new float[min];
            this.f26mY = new float[min];
            ...
            if (((double) Math.abs(this.f25mX[0])) <= 1.0E-5d && ((double) Math.abs(this.f26mY[0])) <= 1.0E-5d) {
                int i3 = min - 1;
                if (((double) Math.abs(this.f25mX[i3] - 1.0f)) <= 1.0E-5d && ((double) Math.abs(this.f26mY[i3] - 1.0f)) <= 1.0E-5d) {
                    int i4 = 0;
                    float f = 0.0f;
                    while (i < min) {
                        float[] fArr2 = this.f25mX;
                        int i5 = i4 + 1;
                        float f2 = fArr2[i4];
                        if (f2 >= f) {
                            fArr2[i] = f2;
            ...
            StringBuilder r0 = outline.m2549("The Path must start at (0,0) and end at (1,1) start: ");
            r0.append(this.f25mX[0]);
            r0.append(",");
            r0.append(this.f26mY[0]);
            r0.append(" end:");
            int i6 = min - 1;
            r0.append(this.f25mX[i6]);
            r0.append(",");
            r0.append(this.f26mY[i6]);
    ...
    private void parseInterpolatorFromTypeArray(TypedArray typedArray, XmlPullParser xmlPullParser) {
        if (TypedArrayUtils.hasAttribute(xmlPullParser, "pathData")) {
            ...
            throw new InflateException(outline.m2556("The path is null, which is created from ", namedString));
    ...
    public float getInterpolation(float f) {
        ...
        float[] fArr = this.f25mX;
        float f2 = fArr[length] - fArr[i];
        if (f2 == 0.0f) {
            return this.f26mY[i];
        }
        float[] fArr2 = this.f26mY;
        float f3 = fArr2[i];
